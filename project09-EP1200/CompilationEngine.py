...
class CompileEngine(object):
    ...
    def _CompileCall(self, subroutineName=None):
        ...
        sym = self._ExpectSymbol('.(') # Kollar om nästa token är "." eller "("
        self._WriteXml('symbol', self.tokenizer.Symbol()) 
        self._NextToken()

        
        if sym == '.': # Om token är "." görs '.' <subroutine-name> '('<expression-list> ')
            objectName = subroutineName  #Fraction
            subroutineName = self._ExpectIdentifier() #new
            self._WriteXml('identifier', self.tokenizer.Identifier())
            self._NextToken()

            if self.symbolTable.KindOf(objectName) != None:
        ...
        if self.symbolTable.KindOf(objectName) == None and objectName == None:
            self.vmWriter.WritePush(SEG_POINTER, 0)

        args = self._CompileExpressionList()
        
        if self.symbolTable.KindOf(objectName) == None: #Constructor
            if objectName == None:
                args += 1 
                self.vmWriter.WriteCall(self.className+"." + subroutineName, args)
            else:
                self.vmWriter.WriteCall(objectName+"." + subroutineName, args)
        elif self.symbolTable.KindOf(objectName) != None:
            args += 1 
            functionname = self.symbolTable.TypeOf(objectName)
            index = self.symbolTable.IndexOf(objectName)
            self.vmWriter.WriteCall(functionname + "."+subroutineName, args)
